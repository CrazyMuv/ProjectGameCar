...
def things_dodged(counting, highest_score, everything_speed):
	...
	score = fnt.render("Очки: " + str(counting), True, green_color)
	h_score = fnt.render("Рекорд: " + str(highest_score), True, green_color)
	speed = fnt.render("Скорость: " + str(everything_speed) + "Km/h", True, green_color)
...
def title_message_display(sh_x, sh_y, color):
	...
	txtSurf, TxtRect = text_objects("Overtaking", lar_txt, color)
...
def crash_function():
	...
	message_display_screen("Авария", 0, 0, red_color, 0)
	while True:
		playAgain = button("Играть", btn_starting_x, nw_gm_y, btn_width, btn_height, greenLight_color, green_color)
		exit_game = button("Выход", btn_starting_x, exit_y, btn_width, btn_height, redLight_color, red_color)
	...
	while welcome:
		for event in pygame.event.get():
			if event.type == pygame.QUIT or exit_game == 1 or (event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
				pygame.quit()
				quit()
		playGame = button("Новая игра", btn_starting_x, nw_gm_y, btn_width, btn_height, greenLight_color, green_color)
		exit_game = button("Выход", btn_starting_x, exit_y, btn_width, btn_height, redLight_color, red_color)
